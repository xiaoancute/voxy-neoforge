...
public class DebugUtils {
                        ...
                        if (lvl == 0) {
                            ...
                            if (own != null) {
                                if (own.getNonEmptyChildren() != msk) {
                                    ...
                                    Logger.error("Section empty child mask not correct " + WorldEngine.pprintPos(own.key) + " got: " + maskString(own.getNonEmptyChildren()) + " expected: " + maskString(msk));
                                    if (tryRepair) {
                                        for (int child = 0; child < 8; child++) {
                                            var section = world.acquireIfExists(lvl - 1, (child & 1) + (x << 1), ((child >> 2) & 1) + (y << 1), ((child >> 1) & 1) + (z << 1));
                                            if (section != null) {
                                ...
                                Logger.error("Section doesnt exist in db but has non empty children " + WorldEngine.pprintPos(WorldEngine.getWorldSectionId(lvl, x, y, z)) + " has children: " + maskString(msk));
                            }
                        }
                    }
                }
    ...
    private static String maskString(byte mask) {
        return String.format("%8s", Integer.toBinaryString(Byte.toUnsignedInt(mask))).replace(' ', '0');
    }
}
